...
public struct BigFloat
{
    ...
    /// </summary>
    private const int divmax = 100;

    private const int sqrtmax = divmax - 1;
    private const int expmax = divmax - 1;
    private const int logmax = 10;
    private const int posinf = -1;
    private const int neginf = -2;
    ...
    public bool IsZero => Radix >= 0 && Value.IsZero;

    public bool IsPositiveInfinity => Radix == posinf;

    public bool IsNegativeInfinity => Radix == neginf;

    public bool IsInfinity => IsPositiveInfinity || IsNegativeInfinity;

    public bool IsNaN => Radix == nan;
    ...
    public BigFloat Reciprocal => new BigFloat(1) / this;

    public static BigFloat Round(BigFloat val)
    {
        if (val.Radix <= 0)
            return val;
        var s = val.Value / BigInteger.Pow(10, val.Radix - 1);
        if (s > 0)
        {
            if (s % 10 >= 5)
                return s / 10 + 1;
    ...
    public static BigFloat Round(BigFloat val, int radix)
    {
        ...
        if (val.Radix > radix)
        {
            var s = val.Value / BigInteger.Pow(10, val.Radix - radix - 1);
            if (s > 0)
            {
                if (s % 10 >= 5)
    ...
    {
        return Parse(bigFloat.ToString().Split('.')[0]);
    }
    ...
    public static BigFloat operator *(BigFloat a, BigFloat b)
    {
        ...
        var returnValue = new BigFloat(a.Value * b.Value, a.Radix + b.Radix);
    ...
    public static explicit operator float(BigFloat value)
    {
        switch (value.Radix)
        {
            ...
            default:
                var res = BigInteger.DivRem(value.Value, BigInteger.Pow(10, value.Radix), out var rem);
    ...
    public static explicit operator double(BigFloat value)
    {
        switch (value.Radix)
        {
            ...
            default:
                BigInteger rem;
                var res = BigInteger.DivRem(value.Value, BigInteger.Pow(10, value.Radix), out rem);
    ...
    public static readonly BigFloat PositiveInfinity = new(0, posinf);

    public static readonly BigFloat NegativeInfinity = new(0, neginf);

    public static readonly BigFloat NaN = new(0, nan);

    public static readonly BigFloat E = new();

    public static readonly BigFloat PI = new(0x64, 0x00, 0x00, 0x00, 0xF7, 0xEF, 0x59, 0xB5, 0x2E, 0xE6,
        0x4C, 0x94, 0xBA, 0x06, 0x54, 0xFA, 0x64, 0xDB, 0xEF, 0x6C, 0x32, 0xEE, 0x91, 0x1D, 0xEB, 0xF6, 0x78, 0x9E,
        0xB0, 0x70, 0xB4, 0x81, 0x87, 0x20, 0x19, 0x01, 0x2B, 0x0D, 0x7D, 0x08, 0xD7, 0xE5, 0x87, 0xEB, 0x73, 0x39);
    ...
    public override string ToString()
    {
        switch (Radix)
        {
            ...
            default:
                var m = this < Zero;
                var str = BigInteger.Abs(Value).ToString("R").PadLeft(Radix + 1, '0');
